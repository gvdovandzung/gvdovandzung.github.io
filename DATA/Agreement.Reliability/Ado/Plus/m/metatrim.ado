*! version 1.0.2  TJS 17mar2000  (STB-58: sbe39.1)
program define metatrim
version 6.0
*
*  syntax: 
*  metatrim varlist [if] [in] [ , Vari CI REffect PRint Funnel EForm
*           LEvel(real 95) ESTimat(R|L|Q) GRaph IDvar(varname) 
...
if "`*'" == "" {
  ...
  di in gr "unnel" in wh "(" in gr "#" in wh ") id" in gr "var" _c
...
syntax varlist(numeric min=2 max=4) [if] [in]  /*
  */ [ , Vari CI REffect Funnel LEvel(real 95) /*
  */     ESTimat(str) GRaph IDvar(varname) SAve(string) * ]
marksample touse
tokenize `varlist'

tempvar theta setheta var zz effect sr delta ceffect order
...
local matname = "Fixed-effects model"
if "`reffect'" == "reffect" { local matname = "Random-effects model" }
...
while `flag' {
  ...
  if "`reffect'" == "reffect" { local d = $S_7 }
                         else { local d = $S_1 }
  qui {
    gen       `delta' = `d'
  ...
  if `i' == 1 {
    ...
    di in gr "Meta-analysis type: " in ye "`matname'"  
    di 
    di in gr "iteration |  estimate    Tn    # to trim     diff"
    di in gr "----------+--------------------------------------"
  }
  di in ye %5.0f `i' in gr "     |" in ye %9.3f `d' %7.0f `Tn' _c
  di in ye %10.0f `trim' %12.0f `sdiff'[_N]

/* 
  di 
  di "         theta      delta    ceffect    sgnrank" _c
  di "         tn       diff" _c
  list `theta' `delta' `ceffect' `sgnrank' `tn' `diff'
*/
