*! version 2.0 May 27, 2008 @ 15:44:22
...
version 8.2
	syntax varlist(min=4 max=4) [if] [in] ///
	  [, Plot Gllamm force IP(string) NIp(int -1)  ///
	...
	di _n(1)   as text "True  positives: " as result "`true1'" ///
	_col(39)   as text "False positives: " as result "`false0'"
	di         as text "False negatives: " as result "`false1'" ///
	_col(39)   as text "True  negatives: " as result "`true0'"
	...
	if "`ip'" == "m" {
		if "`gllamm'" == "" {
			di as error  ///
			  "ip(m) option requires gllamm option"
			exit 198
			}
	...
	if "`force'" == "" {
		qui count if  ///
		  ( (`true1'==0 & `false1'==0) | (`true0'==0 & `false0'==0) ) & `touse'
		if r(N) > 0 {
			di as error "One or more studies have `true1'+`false1'==0 or `true0'+`false0'==0,"
			di as error  ///
			  "i.e. zero total positives or negatives for the reference test."
	...
	qui gen `tf' = _metandi_true / _metandi_n if `touse'
...
*/
			...
			summ `tf' if _metandi_d`g', meanonly
			...
			matrix b`g' = e(b) // 1st element is logit(TF), 2nd is R-E SD 
...
*/
			...
			`quietly' xtmelogit _metandi_true _metandi_d`g' if _metandi_d`g' & `touse', ///
			  nocons ///
			  || _metandi_i: _metandi_d`g' , nocons ///
			  binomial(_metandi_n) ///
			  refineopts(iterate(3)) from(b0uni)

			matrix b`g' = e(b) // 1st element is logit(TF), 2nd is ln(SD)
			qui predict _metandi_u`g' if `touse', reffects
			}
		...
		if "`detail'" !="" {
			di as txt "Covariance of posterior mean logits:"
			correlate _metandi_u1 _metandi_u0 if `touse', covariance
			}
		qui correlate _metandi_u1 _metandi_u0 if `touse'
		drop _metandi_u1 _metandi_u0
		if "`detail'" !="" di as txt "..giving correlation " as res %7.4f r(rho)
		...
		xtmelogit _metandi_true _metandi_d1 _metandi_d0, nocons ///
		  ...
		  binomial(_metandi_n) ///
		  intp(`nip')  `xtmeopts' `log' `trace' ///
		  refineopts(iterate(3)) /// default 2 fails with CT data
		  from(b0) // can pass starting values as vector (undoc'd)
