...
*   - returns for some commands changed in Stata 10
...
program define fitstat, rclass
    ...
    * 12Feb2005 - get bic and aic that stata computes
    if "`nostata'"!="nostata" {
...
*-> compute ll0 for zip and zinb (ll0 is for model with two intercepts)
    ...
    if "`iszi'"=="yes" {
        ...
        * pre 1.7.0        estimates hold `res'
        * 050216 stata 9
        _estimates hold `res'
        quietly `doit'
        sca `llR' = e(ll) /* sic: not ll_0 */
        * pre 1.7.0        estimates unhold `res'
        * 050216 stata 9
...
*-> compute Var(y*) using cov_x
    ...
    if "`isystar'"=="yes" {
        ...
        if "`e(cmd)'"=="tobit"  | "`e(cmd)'"=="cnreg" {
            * pre 1.7.0 mat `v_e'[1,1] = `b'[1,"_se"]*`b'[1,"_se"]
            * 050216 in Stata 9 it is equation sigma: parameter _cons
...
exit
* version 1.8.0 19Jun2006
*   - df correction in lrtest for zinb
* version 1.7.9 26Oct2005
*   fix slogit and mprobit output for LR to Wald X2
...
*  version 1.7.5 10Jun2005
    * clogit, tobit, cnreg, Cragg-Uhler (Nagelkerke) R2 for dif, ztp, ztnb
*  version 1.7.4 10Jun2005
*       align Variance of y*
*  To do: slogit - no lrx2, no bic', are other r's valid?
