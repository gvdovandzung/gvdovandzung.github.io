*! version 1.6.0 - 2/11/00 - initial release for Stata 6  (STB-56: sg145)
*! version 1.6.0 - 2/10/00 - initial release for Stata 6
...
program define fitstat, rclass
    ...
    */  ) { di in r "-fitstat- does not work with the last model estimated."
    ...
    syntax [if] [in] [, Saving(string) Using(string) SAVE Dif FORCE Bic]
    ...
    if "`using'"!= "" {
        ...
        */ ("`fcmd'"=="logistic"&"`e(cmd)'"=="logit") { local same "yes" }
        if ("`fcmd'"!="`e(cmd)'" & "`same'"=="no") & "`force'"=="" {
            di _newline in r "Current model estimated by " in y "`e(cmd)'" /*
