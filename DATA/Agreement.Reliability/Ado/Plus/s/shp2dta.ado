...
void read_shp(string scalar shp_file)
{
	...
	if (type != 1 & type != 3 & type != 5) {
		errprintf("%s: 1point, polyine, or polygon shapefile required\n", shp_file) 
		exit(610)
	}
	...
	if (num_bytes >= `c(memory)') {
		errprintf("insufficient memory\n") 
		errprintf("{p 4 4 2}\n")
		errprintf("To process this data, Stata will need at least\n")
		errprintf("%g megs of memory.\n", num_bytes/(1024^2))
		errprintf("{p_end}\n")
		exit(901)
	}
	...
	while (start_byte < num_bytes) {
		...
		if (type == 1) {
			...
			for (i=1; i<=cols; i++) {
				...
				// Store X and Y observations in `BUFSIZE' block chunks 
				bufsize = `BUFSIZE'
...
void read_dbf(string scalar dbf_file)
{
	...
	fseek(fh_in, num_bytes_header + 1, -1)
