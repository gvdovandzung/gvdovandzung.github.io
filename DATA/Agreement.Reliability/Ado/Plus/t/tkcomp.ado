*! version 2.2.0 - pbe - 9/29/08, 1/11/09 (fix spelling)
program define tkcomp
  version 6.0
  syntax varlist(max=1) [if/] [, Level(real .95) nu(real 0) mse(real 0) dv(varname numeric max=1) ]
  if "`e(cmd)'" ~= "anova" {
    error 301
  }
  quietly capture which qsturng
   ...
   display
   display in green "Tukey-Kramer pairwise comparisons for variable `1'"
