program define tabsort
*! NJC 1.1.0 27 July 1998
    version 5.0
    local varlist "req max(2)"
    local if "opt"
    local in "opt"
    local weight "fweight aweight iweight"
    local options "Missing noRSort noCSort noLAbel Generate(str)"
    local options "`options' Summari(str) SOrt(str) REVerse *"
    parse "`*'"
    parse "`varlist'", parse(" ")
    ...
    * sort on col stats, or carry col variable as is
    ...
    qui while `k' <= `nsort' {
        ...
        if "`sort'" == "freq" {
             ...
             */ sum(`W' * `summari') / sum(1) if `touse'
            by `touse' `var`k'' : replace `stat' = `stat'[_N]
        }
        if "`sort'" == "sd" {
    		by `touse' `var`k'' : replace `stat' = /*
        	 */ sqrt(sum(`W' * (`summari' - `stat')^2) / (sum(1) - 1))
    		by `touse' `var`k'' : replace `stat' = `stat'[_N]
        }
